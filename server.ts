...
// JSON for JSON API calls; raw for multipart (file uploads forwarded as-is)
...
const proxyRequest = async (req: express.Request, res: express.Response, prefix: string) => {
  ...
  if (typeof response.data === 'string') return res.status(response.status).send(response.data);
