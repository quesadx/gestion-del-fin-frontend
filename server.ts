...
async function startServer() {
  ...
  app.post('/api/camps', (req, res) => {
    ...
    const newCamp = {
      ...
      ai_context_prompt:
        ai_context_prompt || 'Establish defensive parameters & optimize starvation metrics.',
    };
    camps.push(newCamp);
  ...
  app.patch('/api/camps/:id', (req, res) => {
    ...
    if (ai_context_prompt !== undefined) camp.ai_context_prompt = ai_context_prompt;
    res.json(camp);
  });
  ...
  app.get('/api/metrics/dashboard', (req, res) => {
    ...
    res.json({
      ...
      low_resource_alerts_count: inventory.filter(
        (i) =>
          i.camp_id === campId &&
          i.quantity < (resources.find((r) => r.id === i.resource_id)?.minimum_stock || 0),
      ).length,
  ...
  app.get('/api/metrics/resources', (req, res) => {
    ...
    const data = resources.map((r) => {
      const inv = inventory.find((i) => i.camp_id === campId && i.resource_id === r.id);
      ...
      return {
        ...
        status:
          qty < r.minimum_stock ? (qty < r.minimum_stock / 2 ? 'CRITICAL' : 'LOW') : 'OPTIMAL',
      };
    });
    res.json(data);
  });
  ...
  app.post('/api/inventory/adjustment', (req, res) => {
    const { camp_id, resource_type_id, resource_id, type, quantity, description } = req.body;
    ...
    if (!resolvedResourceId || isNaN(resolvedQty)) {
      return res.status(400).json({ error: 'resource_id and quantity are required' });
    }

    let inv = inventory.find(
      (i) => i.camp_id === resolvedCampId && i.resource_id === resolvedResourceId,
    );
    if (!inv) {
      inv = {
    ...
    res.status(201).json({
      ...newLog,
      resource_name: resources.find((r) => r.id === resolvedResourceId)?.name || 'Unknown Resource',
    });
  });
  ...
  app.put('/api/people/:id', (req, res) => {
    ...
    if (person) {
      ...
      if (profession_name !== undefined) person.profession_name = profession_name;
  ...
  app.all('/api-remote/*', async (req, res) => {
    ...
    try {
      let response = await makeRequest(
        targetUrl,
        method,
        ['GET', 'HEAD'].includes(method) ? undefined : isMultipart ? req : req.body,
      );
